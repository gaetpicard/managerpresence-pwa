...
function StatistiquesPage() {
  ...
  const [filterPeriode, setFilterPeriode] = useState('all') // 'all', 'month', 'custom'
  ...
  const seancesFiltrees = seances.filter(s => {
    // Vérifier si la séance a des données
      ...
      <Layout title="Statistiques">
        <div className="empty-state">
          ...
          <div className="empty-state-desc">
            Les statistiques apparaîtront une fois des pointages effectués depuis l'application
          </div>
        </div>
      </Layout>
    )
  ...
  return (
    <Layout title="Statistiques">
      ...
      {(filterCreneau ? [creneaux.find(c => c.id === filterCreneau)] : creneauxTries).filter(Boolean).map(creneau => {
        ...
        // Séances où ce créneau est actif
